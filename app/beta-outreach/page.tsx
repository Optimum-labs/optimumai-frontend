...
export default function BetaOutreachPage() {
  return (
    <Suspense>
      <BetaOutreachContent />
    </Suspense>
...
function BetaOutreachContent() {
  ...
  const [projectMessage, setProjectMessage] = useState<{ slug: string; text: string; error: boolean } | null>(null)
  ...
  const handleApply = async (slug: string) => {
    if (!user) {
      ...
      if (!guestData?.fullName || !guestData?.email) {
        setProjectMessage({ slug, text: "Please fill in your name and email to apply.", error: true })
        return
      }
    }
    ...
    try {
      const body: Record<string, string> = { programSlug: slug }
      if (!user) {
      ...
      if (!res.ok) {
        setProjectMessage({ slug, text: data.error ?? "Something went wrong.", error: true })
        if (res.status === 409) setAppliedSlugs((s) => new Set(s).add(slug))
      } else {
        setProjectMessage({ slug, text: "Application submitted! Check your dashboard.", error: false })
        setAppliedSlugs((s) => new Set(s).add(slug))
      }
    } catch {
      setProjectMessage({ slug, text: "Something went wrong. Please try again.", error: true })
    } finally {
      ...
      <main className="optimum-main">
        ...
        <div className="opt-page">
          ...
          <section className="opt-hero">
            <p className="opt-kicker opt-anim-1">Beta Outreach</p>
            <h1 className="opt-headline opt-anim-2">
              The Agency Engine<br />
              <em>Private Beta</em>.
            </h1>
            <p className="opt-sub opt-anim-3">
              Request early access to the first system that measures human agency at scale.
              Challenges, bootcamps, learning resources — one global outreach programme.
            </p>
            <div className="opt-hero-cta opt-anim-4">
              <Link href="/beta-outreach/events" className="opt-btn-primary">
                Upcoming Events <Calendar size={13} />
              </Link>
              <Link href="/beta-outreach/volunteer" className="opt-btn-ghost">
                ...
                <div key={index} className="ed-card">
                  ...
                  <div className="ed-card-body">
                    ...
                    <div className="ed-tags">
                      {project.tags.map((tag, i) => <span key={i} className="ed-tag">{tag}</span>)}
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", gap: "8px", paddingTop: "8px" }}>
                      ...
                      {projectMessage && projectMessage.slug === project.slug && (
                        <span style={{ fontSize: "12px", color: projectMessage.error ? "var(--opt-red)" : "#2a7d4f", fontFamily: "var(--font-dm-mono), monospace" }}>
                          {projectMessage.text}
                        </span>
                      )}
                    </div>
          ...
          {/* ── Community Voices ── */}
          <div className="opt-rule" style={{ marginTop: "64px" }}><span className="opt-rule-text">Beta Voices</span></div>
